...
def promote_behavior(runtime, draft, analysis, sandbox_run, behavior_fn, approval_policy=True):
    ...
    runtime.behaviors[binding_id] = BoundBehavior(binding_id, draft.id, draft.name, draft.declared_trigger_events[0], draft.declared_scope,
                                                  {"max_emitted_events": 10}, True, behavior_fn)
    ev = Event("behavior.bound", {"binding_id": binding_id, "draft_id": draft.id})
    runtime.apply_event(ev)
    return PromotionDecision(str(uuid.uuid4()), draft.id, "approved", "all gates passed", draft.declared_scope, {"max_emitted_events": 10}, "policy", ev.id)
