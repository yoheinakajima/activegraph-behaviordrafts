...
def _build_prompt(goal: Dict[str, Any], fixture: Dict[str, Any]) -> str:
    return (
        ...
        "  - Avoid nested quotes inside f-strings.\\n"
        "  - Prefer assigning object id to a local variable first: obj_id = obj[\"id\"]; summary_id = f\"summary-{obj_id}\".\\n"
        "  - Do not write: f'summary-{obj['id']}'.\\n"
        "  - If using f-strings, use a local variable rather than indexing inside the f-string.\\n"
        ...
        "  obj_id = obj[\"id\"]\\n"
        "  summary_id = f\"summary-{obj_id}\"\\n"
        "  ctx.emit_object_created({\"id\": summary_id, \"type\": \"Summary\"})\\n"
        "  ctx.emit_relation_created({\"type\": \"summarizes\", \"from\": summary_id, \"to\": obj_id})\\n"
        "- Minimal object-only pattern:\\n"
        "  obj = event[\"object\"]\\n"
        "  obj_id = obj[\"id\"]\\n"
        "  warning_id = f\"warning-{obj_id}\"\\n"
        "  ctx.emit_object_created({\"id\": warning_id, \"type\": \"Warning\"})\\n"
